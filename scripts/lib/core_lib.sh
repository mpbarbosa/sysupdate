...
# True when SUDO_ASKPASS names an executable helper: something that can hand
# sudo a password without a terminal (the web bridge sets one that relays the
# prompt to the dashboard; a desktop may set ssh-askpass or similar).
sudo_askpass_available() {
    [ -n "${SUDO_ASKPASS:-}" ] && [ -x "$SUDO_ASKPASS" ]
}

# Returns 0 when a sudo command can succeed without a manual password prompt
# being impossible: credentials are already cached, there is a TTY on stdin to
# prompt against, or an askpass helper is available. In non-interactive
# contexts (e.g. the web backend) with none of these this returns 1, letting
# callers skip work early instead of failing after side effects such as
# downloads.
sudo_can_run() {
    sudo_credentials_cached || [ -t 0 ] || sudo_askpass_available
}
...
emit_sudo_required_event() {
    ...
    if sudo_askpass_available; then
        askpass="true"
    fi
...
# Without a terminal, sudo only consults SUDO_ASKPASS when told to with -A:
# original sudo falls back to the helper on its own, sudo-rs (Ubuntu 25.10+)
# refuses with "A terminal is required to authenticate". The updater and its
# snippets call plain `sudo` in ~90 places, and some hand off to installer
# scripts that call it themselves, so rather than threading -A through every
# call site this puts a one-line `sudo` wrapper first on PATH that adds -A and
# execs the real binary. Only engaged when there is no TTY and a helper exists;
# an interactive run keeps prompting on the terminal as before.
enable_sudo_askpass_shim() {
    if [ -t 0 ] || ! sudo_askpass_available; then
        return 0
    fi

    local shim_dir="$SYSUPDATE_STATE_DIR/askpass-shim"
    local real_sudo="" dir
    local IFS=':'
    for dir in $PATH; do
        [ -n "$dir" ] && [ "$dir" != "$shim_dir" ] || continue
        if [ -x "$dir/sudo" ] && [ ! -d "$dir/sudo" ]; then
            real_sudo="$dir/sudo"
            break
        fi
    done
    ...
    mkdir -p "$shim_dir" 2>/dev/null && chmod 700 "$shim_dir" 2>/dev/null || return 1
    # Regenerated every run: the real sudo path or state dir may have moved.
    # shellcheck disable=SC2016  # $SUDO_ASKPASS / "$@" are meant for the shim, not this shell
    printf '#!/bin/sh\n# Generated by sysupdate (core_lib.sh::enable_sudo_askpass_shim). Do not edit.\n# Forces sudo to read the password via $SUDO_ASKPASS instead of a terminal.\nexec %s -A "$@"\n' \
        "$(printf '%q' "$real_sudo")" > "$shim_dir/sudo" 2>/dev/null || return 1
    chmod 700 "$shim_dir/sudo" 2>/dev/null || return 1
    ...
    print_status "No terminal: sudo password prompts are routed through the askpass helper"
    return 0
}
...
run_with_sudo() {
    ...
    if ! sudo_can_run; then
        ...
        print_error "Re-run in an interactive terminal, run 'sudo -v' to cache credentials, or set SUDO_ASKPASS before using non-interactive/quiet mode"
        return 1
    fi
